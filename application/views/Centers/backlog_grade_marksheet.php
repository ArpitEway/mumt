...
<table class="table mb-0">
    <tbody>
        ...
        <!-- <tr>
            ...
            <th class="border-top-0" colspan="3"><?php  echo $student_info->center_name.' ( '.$student->center_code.' ) '; ?></th>
        </tr> -->
    </tbody>
</table>
 <table class="table mb-0" >
      <tbody>
        ...
        $gradesheetData = $this->Gradesheet_backlog_model->view_result($student->student_id,$student->course_group_id,$student->class_id,$student->mode,$student->id);
    ...
    <table class="border border-dark m-auto w-100" >
      ...
      if($classData->last_class == 'L' && (!in_array($student->center_id,$dept_ids))){
        ...
        foreach($classes as $cls){
            ...
            $gradeData = $this->Gradesheet_model->view_old_results($student->student_id,$student->course_group_id,$old_result[0]->class_id,$student->mode, $old_result[0]->id, $old_result[0]->exam_status);
        ...
        <tr>
          ...
          <td class="text-center" style="vertical-align: middle;"><?= (count($attemp_count)==0 || count($attemp_count)==1)?'Two': $wordNumerals[count($attemp_count)]?></td>
        </tr>
        </table><br>
        <table class="border border-dark m-auto w-100" >
        <tr>
        ...
        <?php
          ...
          (count($attemp_count)==0 || count($attemp_count)==1)?'Two': $wordNumerals[count($attemp_count)]
        //   (count($attemp_count) == 0)?1:count($attemp_count)?></td>
