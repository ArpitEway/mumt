...
$menus = $this->db->get_where('menu', array('id' => $param1))->result_array();

foreach($menus as $menu): ?>

<form method="POST" class="d-block ajaxFormEdit" action="<?php echo site_url('admin/Admins/add_menu/update/'.$menu['id']); ?>">
...
<input type="hidden" class="csrfname" name="<?= $name_csrf; ?>" value="<?= $hash_csrf; ?>">
		  <div class="form-group col-md-4">
				...
				<option value="all">Select admin</option>
					...
					<?php
					$admins = $this->db->get_where('admin_master', array())->result_array();
					foreach($admins as $admin)
					{
					?>
					
					<option value="<?php echo $admin['id']; ?>" <?php if($admin['id'] == $menu['admin_id']){ echo "selected"; } ?>>
        ...
        <div class="form-group col-md-4">
			...
			<?php
					$menu_headings = $this->db->get_where('menu_heading', array('admin_id' => $menu['admin_id']))->result_array();
...
<script>
	...
	$("#edit_menu_submit").on('click',function (e){
		...
		var frm = $('.ajaxFormEdit').serialize();
