...
foreach($elist as $row)
{
      ...
      $sql="SELECT count(*) as cnt FROM `new_exam_form_report` as `e` JOIN `student_report` as `s` ON `e`.`student_id` = `s`.`student_id` AND   `s`.`class_id` = `e`.`class_id` WHERE  `s`.`examcentercode`='".$row->examcentercode."'   AND  `e`.`paper_code` = '".$paperData[0]['paper_code']."' AND `s`.`class_id` = '".$paperData[0]['class_id']."' AND   `s`.`exam_center_id` = '".$row->id."'   AND (new_exam_form!='D' OR ( `s`.`session` = 'July 2022' AND `s`.`class_name` = 'I Year' ) OR ( `s`.`session` = 'Jan 2023' AND `s`.`class_name` = 'I SEM' ));";    
    //   AND s.class_id not in (163,175,153,155,182,299,161,216,214,159,154,158,181,172,160,152)
    $query = $this->db->query($sql);
    $count = $query->result_array();

    //back 
    $sql_back="SELECT count(*) as cnt FROM `backlog_exam_form_report` as `e` JOIN `backlog_student_report` as `s` ON `e`.`student_id` = `s`.`student_id` AND   `s`.`class_id` = `e`.`class_id`  AND s.id=e.backlog_student_id WHERE  `s`.`exam_center_code`='".$row->examcentercode."'   AND  `e`.`paper_code` = '".$paperData[0]['paper_code']."' AND `s`.`class_id` = '".$paperData[0]['class_id']."' AND   `s`.`exam_center_id` = '".$row->id."'  AND exam_form!='D' AND `e`.`status`='B'  AND s.exam_year='June 2023'";    
    // AND s.class_id not in (163,175,153,155,182,299,161,216,214,159,154,158,181,172,160,152)
    $query_back = $this->db->query($sql_back);
    $count_backlog = $query_back->result_array();
    
   
        $qu="SELECT count(*) as num FROM `student_report` as s join paper_master as p on s.class_id=p.class_id WHERE  `p`.`paper_code` = '".$paperData[0]['paper_code']."' AND `p`.`class_id` = '".$paperData[0]['class_id']."' AND `s`.`examcentercode`='".$row->examcentercode."' AND   `s`.`exam_center_id` = '".$row->id."'  AND temp_exam_form='N' and `session` = 'July 2022' AND `s` .`class_name` = 'I Year'  ";
        // AND s.class_id not in (163,175,153,155,182,299,161,216,214,159,154,158,181,172,160,152)
    ...
    <tbody><tr><td colspan="4" align="center"><h4 align="center">TOP SECRET</h4></td></tr>
        ...
        <tr>
            ...
            <td><?php  
                    ...
                    if($paperData[0]["exam_shift"]=='Morning'){echo "10:30 AM To 01:30 PM";}
                    if($paperData[0]["exam_shift"]=='Afternoon'){echo "02:30 PM To 05:30 PM"; }  ?> 
            </td> </tr> <tr>
