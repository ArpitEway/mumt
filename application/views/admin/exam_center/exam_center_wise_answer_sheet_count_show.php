...
$total = 0;
    ...
    {
        /*****************/
		$sql="SELECT count(*) as cnt FROM `new_exam_form` as `e` JOIN `student` as `s` ON `e`.`student_id` = `s`.`student_id` AND   `s`.`class_id` = `e`.`class_id` WHERE     `s`.`exam_center_id` = '".$row->id."' AND   course_complete='N'  AND  paper_type='theory' AND  (new_exam_form!='D' OR ( `s`.`session` = 'July 2023' AND `s`.`class_name` = 'I Year' AND payment_status='Y' ) OR ( `s`.`session` = 'Jan 2024' AND `s`.`class_name` = 'I SEM' ))";
        //new_exam_form in ('Y') 
        ...
        $sql_back="SELECT count(*) as cnt FROM `backlog_exam_form` as `e` JOIN `backlog_student` as `s` ON `e`.`student_id` = `s`.`student_id` AND   `s`.`class_id` = `e`.`class_id` AND s.id=e.backlog_student_id  WHERE  `s`.`exam_center_code`='".$row->examcentercode."'   AND   `s`.`exam_center_id` = '".$row->id."'  AND exam_form not in ('D') and s.exam_year='June 2024' AND `e`.`status`='B'";    
