...
$total = 0;
    foreach($exam_centers as $row)
    {
        ...
        // and new_exam_form in ('Y') 
        //  AND  (new_exam_form!='D' OR ( `s`.`session` = 'July 2024' AND `s`.`class_name` = 'I Year' ))
        ...
        $sql_back="SELECT count(*) as cnt FROM `backlog_exam_form` as `e` JOIN `backlog_student` as `s` ON `e`.`student_id` = `s`.`student_id` AND   `s`.`class_id` = `e`.`class_id` AND s.id=e.backlog_student_id  WHERE  `s`.`exam_center_code`='".$row->examcentercode."'   AND   `s`.`exam_center_id` = '".$row->id."'  AND exam_form ='Y' and s.exam_year='June 2025' AND `e`.`status`='B'";    
