   ...
   <table width="95%" align="center" cellspacing="0" cellpadding="8" border="1">
     ...
     // $this->db->where_in('class_id',array(141,150,186) );
      ...
      { 
        ...
        $sql="SELECT count(*) as cnt FROM `new_exam_form` as `e` JOIN `student` as `s` ON `e`.`student_id` = `s`.`student_id` AND   `s`.`class_id` = `e`.`class_id` WHERE  `s`.`exam_center_id`='".$exam_center."'   AND  `e`.`paper_code` = '".$paper->paper_code."' AND `s`.`class_id` = '".$paper->class_id."' AND   `s`.`exam_center_id` = '".$exam_center."' AND new_exam_form ='Y' and s.class_id not in (166,167,267,164,165,162,163,169,170,171,174,175,177,178,168,180,173,269,152,154,158,160,172,181,215) ";
      ...
      $sql_back="SELECT count(*) as cnt FROM `backlog_exam_form` as `e` JOIN `backlog_student` as `s` ON `e`.`student_id` = `s`.`student_id` AND   `s`.`class_id` = `e`.`class_id` AND s.id=e.backlog_student_id WHERE  `s`.`exam_center_id`='".$exam_center."'   AND  `e`.`paper_code` = '".$paper->paper_code."' AND `s`.`class_id` = '".$paper->class_id."' AND   `s`.`exam_center_id` = '".$exam_center."' AND exam_form='Y' AND `e`.`status`='B' and s.class_id not in (166,167,267,164,165,162,163,169,170,171,174,175,177,178,168,180,173,269,152,154,158,160,172,181,215) AND s.exam_year='Dec 2023'";
