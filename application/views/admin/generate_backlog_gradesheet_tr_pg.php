...
//    print_r($student);die;
   $student_info = $this->Common_model->getRecordById('student','student_id',$student->student_id);
    ...
    //   echo $new_exam_form->p_marks;die;
      ...
      if($new_exam_form->int_marks=='N' && $classData->practical_internal_marks=="Y" && $student->mode != 'PVT' && $new_exam_form->max_internal_marks !=0){
        $rwas_count++;
      }
    }
    ...
    if($page_break_count%2==0 || $page_break_count==0 || $previous_center!=$current_center){
      ...
      <table class="table table1 mb-0">
        ...
        <?php }  
        if(($classData->practical_internal_marks!='N') && $student->mode != 'PVT'){
          // echo $student->mode;die;
        ?>
        <tr>
...
<?php
   $gradesheetData = $this->Gradesheet_backlog_tr_model_pg->view_result($student->student_id,$student->course_group_id,$student->class_id,$student->mode, $student->id);
  ...
  ?>
    ...
    <table class="table table1">
     ...
     <td  class="align-middle text-center total">Total</td>
          ...
          <td  class="align-middle text-center result" rowspan="<?php echo $rowspandata ?>"><?php  if($gradesheetData['result'] == 'RW'){echo '';}else{ echo ($gradesheetData['result'] == 'FAIL' || $gradesheetData['result'] == 'ATKT')?'0.00':number_format((float)$gradesheetData['agpa'], 2, '.', '');} ?></td>
...
<?php } ?>
<?php if( ($classData->practical_internal_marks!='N') && $student->mode != 'PVT'){ ?>
  <tr>
...
<?php } ?>
  <tr>
    ...
    //   } else
      if(($paper_master->theory_marks<$paper_master->min_theory_marks) || $paper_master->theory_marks=='ABS' || $paper_master->int_marks=='ABS'){
  ...
  <?php
   $ddd = $this->Gradesheet_backlog_tr_model_pg->view_result($student->student_id,$student->course_group_id,$student->class_id,$student->mode,$student->id);
...
foreach($classes as $cls){
 ...
 foreach($old_result as $old){
   $old_grade_data =  $this->GradeSheet_old_model_pg->view_old_results($student->student_id,$student->course_group_id,$old->class_id,$student->mode,$old->id);
