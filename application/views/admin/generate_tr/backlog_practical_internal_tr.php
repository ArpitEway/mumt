...
<?php
  ...
  foreach($students as $student)
  {
    ...
    if($page_break_count%4==0 || $page_break_count==0 || $previous_center!=$current_center){
      ...
      <p align="center" class="line-height">Tabulation Register for <strong><?php echo $student_info->course_name; echo '&nbsp'. $course_duration; ?></strong> <?php echo ' Backlog '.$marksheetData[0]->exam_session.' (CBCS)';?>
      </p>
      <div>
      ...
      <table class="table table1 mb-0">
        ...
        <tr>
          <td class="align-middle text-right"><?=($classData->project=='Y') ? 'Project' : 'Practical' ?> Internal Marks Max/-></td>
          <?php foreach($marks as $paper_master){   ?>
          <td  class="align-middle text-center">
            <?php if($paper_master->paper_type!="theory"){ if($paper_master->max_internal_marks !=0){echo  $paper_master->max_internal_marks;}else{ echo '';};}  ?>
          </td>
          <?php } ?>
          <td class="align-middle text-center"></td>
        </tr>
        ...
        <tr>
          ...
          foreach($marks as $paper_master){ 
            $count = count($marks)-2;  
            $paper_codes=array('1RBBA1','1RBBA2','1RCBBA1','1RCBBA2','1RBA1','1RBA2','1RBCOM1','1RBCOM2','1RBCOMCA1','1RBCOMCA2','1RBCOMT1','1RBCOMT2','1RBCA1','1RBCA2','1RBSCCBC1','1RBSCCBC2','1RBSCCS1','1RBSCCS2','1RBSCPCM1','1RBSCPCM2','1RBSW1','1RBSW2','2RBBA1','2RBBA2','2RBA1','2RBA2','2RBCOM1','2RBCOM2','2RBCOMCA1','2RBCOMCA2','2RBCOMT1','2RBCOMT2','2RBCA1','2RBCA2','2RBSCCBC1','2RBSCCBC2','2RBSCCS1','2RBSCCS2','2RBSCPCM1','2RBSCPCM2','2RBSW1','2RBSW2','3RBBA1','3RBBA2','3RBA1','3RBA2','3RBCOM1','3RBCOM2','3RBCOMCA1','3RBCOMCA2','3RBCOMT1','3RBCOMT2','3RBCA1','3RBCA2','3RBSCCBC1','3RBSCCBC2','3RBSCCS1','3RBSCCS2','3RBSCPCM1','3RBSCPCM2','3RBSW1','3RBSW2');
...
<?php
   $gradesheetData = $this->Gradesheet_backlog_tr_model->view_result($student->student_id,$student->course_group_id,$student->class_id,$student->mode,$student->id);
    ...
    foreach($marks as $new_exam_form)
    {
      ...
      if($new_exam_form->int_marks=="N"){
        ...
        if($new_exam_form->int_marks < $new_exam_form->min_internal_marks || $new_exam_form->int_marks=='ABS'){
          ...
          echo  $new_exam_form->int_marks.' C';
...
<?php } ?>
  <tr>
    ...
    <?php foreach($marks as $paper_master){ 
      ...
      if($paper_master->p_marks=='ABS'){
        ...
        echo $paper_master->p_marks+$paper_master->int_marks.$status;
      }
    } ?>
    </td>
    <?php } ?>
  ...
  <?php
   $this->Gradesheet_backlog_tr_model->view_result($student->student_id,$student->course_group_id,$student->class_id,$student->mode,$student->id);
  ...
  <?= 'Session'.'<br>'.'Sem/Year'.'<br>'.'Roll no'.'<br>'.'Marks'.'<br>'.'Credit Earned / Points '.'<br>'.'AGPA'?></strong>
...
foreach($classes as $cls){
 ...
 foreach($old_result as $old){
    $old_grade_data   = $this->Gradesheet_model->view_old_results($student->student_id,$student->course_group_id,$old->class_id,$student->mode, $old->id, $old->exam_status);
...
<td class="align-middle text-center "  colspan="3"><strong>Grand Total</strong><br><?= $total_ob.'/'.$total_mar?><br><strong>Total Credit</strong><br><?= $total_course_credit?><br><strong>CGPA</strong><br><?= $cgpa?></td>
