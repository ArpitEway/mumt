...
<div class="container mt-3" >
	<table id="kt_datatable" class="table table-striped dt-responsive nowrap " width="100%" >
		...
		<?php
		$i = 1;
        $accounts = $this->db->get_where('support_system', array())->result_array();
					...
					<tr>
                	   ...
                	   <td>
                		<div style="display: inline-flex;">
                			<a href="javascript:void(0);" class="dropdown-item" onclick="rightModal('<?php echo site_url('admin/modal/popup/admin/support/edit_support/'.$acc['id']); ?>', '<?php echo 'Update account' ?>')"> <i class="mdi mdi-pencil edit-icon"></i></a>   
                			<a href="javascript:void(0);" class="dropdown-item" onclick="confirmModal('<?php echo site_url('support_system/delete/'.$acc['id']); ?>', showAllaccount )"><i class="mdi mdi-delete delete-icon"></i></a>
                		</div>			
                    </td> 
					</tr>
...
$(document).on('change', '.status_checks', function() {
  ...
  $("#reason").html("");
		...
		if($(this).is(":checked") == true)
		{
			...
			var url = BASE_URL + "admin/Admins/update_support_status";
    ...
    {
        var url = '<?php echo site_url('admin/Admins/support_system'); ?>';
        $.ajax({
            type : 'GET',
            url: url,
