      ...
      foreach($students as $row){ 
        ...
        $absCount = $this->Common_model->getCountByWhere('old_result_data',array('student_id' => $row->student_id,'class_id'=>$row->class_id,'type'=>'theory','theory_marks'=>'ABS'));
          ...
          <td> <?php  if($paperCount==$absCount){?> 
            <a class="text-danger" href="<?=base_url('admin/scripts/Postexam/diploma_demo_set/'.$row->student_id.'/'.$row->class_id)?>">Set Demo</a> <?php } else{?>
             <a class="text-info" href="<?=base_url('admin/scripts/Postexam/diploma_backlog_marks_update_scripts/'.$row->student_id.'/'.$row->class_id)?>">Set Backlog</a> <?php } ?>  
           </td>   
         </tr> 
         <?php 
         $c =1;
