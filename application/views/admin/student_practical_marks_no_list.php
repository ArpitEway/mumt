			...
			foreach($students as $student){
				?>
				   <tr>
					...
					<td><?php echo $this->Common_model->getClassNameByClassId($student->old_class_id); ?></td>
					...
					if($student->p_marks_sub=='N'){
						...
						<button style="display: none" class="btn btn-info btn-sm font-weight-bold view" id="<?="roll_num{$student->student_id}"; ?>" data-toggle="modal" data-target="#kt_datepicker_modal"  data-id="<?=$student->student_id;?>" onclick="view_mark(<?=$student->student_id;?>,<?=$student->old_class_id; ?>)">view</button>
					...
					<?php }
						...
						<button  class="btn btn-primary btn-sm font-weight-bold student" id="<?="roll_{$student->student_id}"; ?>" data-toggle="modal" data-target="#kt_datepicker_modal"  data-id="<?=$student->student_id;?>" onclick="mark_submission(<?=$student->student_id;?>,<?=$student->old_class_id; ?>)">Edit</button>
...
<script>
		...
		function mark_submission(student_id,old_class_id){
			...
			$.ajax({
				type: "POST",
				url: BASE_URL+"admin/admins/student_practical_assignment_marks",
				dataType:"JSON",
		...
		function view_mark(student_id,old_class_id){
			...
			$.ajax({
				type: "POST",
				url: BASE_URL+"admin/admins/view_student_marks",
				dataType:"JSON",
				data: {student_id: student_id,old_class_id: old_class_id, [csrfName]:csrfHash},
				success: function(response){
					$('.modal-body').html(response.data);
