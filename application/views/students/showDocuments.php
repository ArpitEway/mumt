					...
					<label class="w-100"><?=$document['document'];?><strong class="text-danger"><?= ($document['status']=='N') ? '' : ' *'; ?></strong>
						<!-- <span class="float-right" id="<?='downloadBtnId_'.$document['id']?>"> -->
							...
							if($file){
								...
								<a data-magnify="gallery" data-src="" data-caption="<?=$document['document'];?>" data-group="a" href="<?=base_url($src);?>">
								<?=$document['document'];?>
								</a>
...
<script>
	$(".custom-file-input").on('change',function (){
		...
		if(data==''){
			...
			if(filesize>500){
				...
				$.ajax({
					url: "<?=base_url('center/StudentDocument/uploadDoc');?>",
					type: 'POST',
	...
	var student_id = $('#student_id').val();
		...
		var csrfHash = $('.csrfname').val(); 
		$.ajax({
			url: "<?=base_url('center/StudentDocument/checkDocumentStatus');?>",
			type: 'POST',
			dataType : 'json',
