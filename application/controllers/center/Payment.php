...
class Payment extends CI_Controller {
   ...
   public function backlog_exam_form($student_id,$class_id,$back_id){
    ...
    public function backlog_response(){
		...
		$student = $this->Common_model->getRecordByWhere('backlog_student',array('student_id'=>$student_id,'class_id'=>$class_id,'exam_year'=>'June 2025'));
       $student_name =  $this->Common_model->getSinglefield('student','name',array('student_id'=>$student_id));
			...
			if($payment=='Y'){
				$where = array('student_id'=>$student_id,'class_id'=>$class_id,'exam_year'=>'June 2025');
				$student = array($status=>'Y');
	...
	public function application($student_id,$apply){

		if(!$this->session->has_userdata('centerdata')){
			redirect(base_url('center/login'));
		}
		$titleData = array('title'=>'Application Payment');
		$student_id = $this->Common_model->encrypt_decrypt($student_id,'decrypt');
		$student = $this->Common_model->student_info($student_id);
	
		$txnAmt = $this->Common_model->getRecordByWhere("application_field",array('field'=>$apply));
		...
		$data['student'] = $student;
		$data['url'] = 'paynow';
		$data['paymentType'] = 'Appliction('.$apply.')';
		$data['txnAmt'] = $txnAmt[0]->amount;
		$data['apply'] = $apply;
		// $mode = $this->input->post('mode');
		// if($student['university_mode']=='REG'){
		...
		$this->load->view('Centers/header',$titleData);
		$this->load->view('Centers/application_payment',$data);
		$this->load->view('Centers/footer');
	}

	public function application_payment($student_id,$apply){
		if(!$this->session->has_userdata('centerdata')){
			redirect(base_url('login'));
		}
		$student_id = $this->Common_model->encrypt_decrypt($student_id,'decrypt');
		...
		/*  live credential  */
			...
			$posted['key'] = $MERCHANT_KEY;
			$posted['txnid'] = $txnid; 
			$posted['surl'] =base_url('center/payment/application_response');
			$posted['furl'] =base_url('center/payment/application_response');
			$posted['amount'] =$txnAmt;
			$posted['firstname'] = $student['name'];
			$posted['email'] = $student['p_email'];
			$posted['phone'] = $student['p_mobile_no'];
			$posted['productinfo'] = $apply;
			$posted['address1'] = $student['p_address'];
			$posted['city'] = $student['p_city'];
			$posted['state'] = $student['p_state'];
			$posted['country'] = $student['nationality'];
	...
	public function application_response(){
    //    print_r($_POST);die;
		$date = date('Y-m-d'); 
		$time = date('h:i:s');
		$student_id=$_POST["udf1"];
		...
		$hash = hash("sha512", $retHashSeq);
		 if ($hash != $posted_hash) {
			...
			$where = 'student_id='.$student_id.' and fees_head="'.$productinfo.'" and class_id='.$student->class_id;
			$txnData = $this->Common_model->get_record('online_payment_transaction','*',$where);
                if($txnData[0][['payment_status']] != "success"){
				...
				if($payment=='Y'){
					// $where = 'student_id='.$productinfo;
					$where = 'enrollment="'.$student->enrollment_no.'" and apply_for="'.$productinfo.'"';
					$student = array('payment'=>'Y');
					$this->Common_model->updateRecordByConditions('application_form',$where,$student);
				}
