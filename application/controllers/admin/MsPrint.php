...
class MsPrint extends CI_Controller {
	...
	public function marksheet($exam_data_id="")
	{
		...
		if ($data['exam_data']->exam_status == "B") { //Backlog
			...
			}elseif ($data['exam_data']->marks_pattern=='GRADE' && $data['exam_data']->university_mode == "REG" &&  ($class->cbcs=='Y' || in_array($class->id, [267,268,312,313]))) {
                $this->load->model('GradeSheet_old_model_pg');
				if(in_array($class->id, [267,268,312,313])){
