   ...
   public function assign_answersheet_sub(){
	...
	public function testid_wise_student_count(){
		...
		}else
		{
			...
			// $this->db->where_in('class_id',array(101,102,103,104,105,106,107,108,109,110,111,112,116,117,118,119,120,121,125,126,127,128,129,130,131,132,133,134,135,136,137,138,139,140,141,142,143,144,145,146,147,148,149,150,151,153,155,159,161,162,163,164,165,168,169,170,171,173,174,175,177,178,180,182,183,184,185,186,187,188,189,190,191,192,194,196,198,200,202,204,206,208,210,212,214,216,218,222,224,226,228,230,232,234,236,238,240,242,244,246,248,250,252,254,256,258,260,262,264,268,270,273,274,276,278,280,282,283,284,285,286,287,288,289,290,291,292,293,294,295,296,297,298,299,300,301,303,305,310,311));
			
			// $this->db->where_not_in('class_id',array(154,172,181));106,109,136,148

			$this->db->where_in('class_id',array(172,181,193,195,197,199,201,203,205,207,209,211,213,215,217,221,223,225,227,229,231,233,235,237,239,241,243,245,247,249,251,253,275,277,279,281,302,304,404,406,408,410,412,414,416,418,420,422,424,426,428,430,432,434,436,438,440,442,444,446,448,450,452,454,456,458,460,462,464,466,468,470,472,474,476,478,480,482,484,486,488,490,492,497,502,504,506,508,510,512,155,182,154));
	...
	public function testid_wise_student_count_multiple(){
		...
		//	$this->db->where('exam_date!=','0000-00-00');106,109,136,148
	...
	public function envelope_cover_page_multiple_testid(){
		...
		}else
		{
			...
			$this->db->where('exam_date!=','0000-00-00');
			// $this->db->where('exam_date!=','');
			// $this->db->where_in('test_id',array(5001,5002,6395));
			$this->db->group_by('test_id ');
			$this->db->having(' tot>1');
			$this->db->order_by("test_id", "asc");
			$data['list'] = $this->db->get()->result();
	...
	public function exam_center_wise_paper_count(){
		...
		}else
		{
			...
			$this->db->where_in('class_id',array(194,196,198,200,202,204,206,208,210,212,214,303,276,280,222,224,226,228,101,104,107,110,116,119,125,128,131,134,162,163,164,165,168,169,170,171,173,174,175,177,180,139,142,145,148,151,184,186,188,190,192,183,185,187,189,191,138,141,144,147,150,137,140,143,146,149));
...
/*
		...
		// echo $this->load->view('admin/exam_center/exam_center_paper_count_show_pvt',$data, TRUE);
	}
	...
	public function exam_center_wise_student_attendance_sheet(){
		...
		}else
		{
			...
			$where = array('new_exam_form'=>'Y', 'roll_no!=' => 0 ,'notification_no'=>23);
			$this->db->where($where);
	...
	public function get_exam_center_wise_student_attendance_sheet(){
		...
		// if($exam_center!="All")
		$where = array('exam_center_id'=>$exam_center,'new_exam_form'=>'Y', 'roll_no!=' => 0 ,'notification_no'=>23);
		$this->db->where($where);	
	...
	public function exam_center_wise_backlog_student_attendance_sheet(){
		...
		}else
		{
			...
			$this->db->where('notification_no','23');
	...
	public function get_exam_center_wise_backlog_student_attendance_sheet(){
		...
		$where = array('backlog_student.exam_center_id'=>$exam_center,'backlog_student.exam_form'=>'Y', 'backlog_student.roll_no!=' => 0 ,'backlog_student.notification_no'=>'23','backlog_student.exam_year'=>'Dec 2025');
		$this->db->where($where);	
		$data['exam_center_students'] = $this->db->get()->result();
...
/*
	...
	public function show_examcenter_folio(){
		if($_POST['action']=='assign_examcenter'){
			...
			$dataArray['class_name']= $this->Common_model->getClassNameByClassId($_POST['class_id']);
			// $this->db->where('exam_date!=',"");
			if(in_array($_POST['class_id'], $classes) && $_POST['university_mode']=='PVT'){
				$this->db->where('pvt_exam_date!=',"0000-00-00");
			}else{
				$this->db->where('exam_date!=',"0000-00-00");
			}
			...
			$dataArray['examSession']="December 2025";
     ...
     public function class_wise_result_upload_status(){
		...
		if(!$this->session->has_userdata('adminData')){
         ...
         //  array('exam_form_permission' => 'Y')
			...
			// $this->db->where_in('class_id',array(193,217,197,231,203,237,211,275,277,279,281,221,247,223,249,225,251,263));
	...
	public function class_wise_backlog_result_upload_status(){
		...
		if(!$this->session->has_userdata('adminData')){
			...
			$course_group = $this->Common_model->get_record('backlog_student','DISTINCT(class_id) as  class_id, course_group_id' ,array('exam_form'=>'Y','exam_year'=>'Dec 2025'));
...
public function getStudentData()
{
	...
	public function show_counter_folio_check(){
		if($_POST['action']=='assign_examcenter'){
			...
			$dataArray['class_name']= $this->Common_model->getClassNameByClassId($_POST['class_id']);
			//$this->db->where('exam_date!=',"");
			if($_POST['university_mode']=='PVT' && in_array($_POST['class_id'],array(104,107,134))){
				$this->db->where('pvt_exam_date!=',"0000-00-00");
			}else{
				$this->db->where('exam_date!=',"0000-00-00");	
			}
			...
			//,'result_show'=>'Y'
				...
				if (count($student)==0) {
					...
					if($student[0]->result_show =="N"){
						...
						$data['exam_session']  = 'January 2026';
						/**********************/
	...
	public function getEditStudentMarksDataWH()
	{
		...
		/*	if($studentData[0]->university_mode == "REG"){
			$qry = $this->db->query("SELECT * FROM `new_exam_form` as e join paper_master as p on p.id=e.paper_id and p.paper_code=e.paper_code WHERE `student_id`=".$studentData[0]->student_id." AND p.class_id=".$studentData[0]->old_class_id." and e.class_id=".$studentData[0]->old_class_id." and paper_type='Theory' and e.theory_marks<p.min_theory_marks
			 ");
			}else{
	...
	{
		$roll_no = $this->input->post('roll_no');
		$where = array('backlog_student.roll_no'=>$roll_no,'backlog_student.exam_form'=>'Y','backlog_student.exam_year'=>'Dec 2025');
	...
	public function edit_backlog_student_marks_sub()
	{
		...
		foreach ($paper_code as $key => $id) {
			$where = array('paper_code' => $id, 'student_id' => $student_id, 'class_id' => $class_id ,'backlog_student_id' => $this->input->post('backlog_student_id'));
			$data = array('theory_marks' => $theory_marks[$key]);
			if (isset($_POST['int_marks'])) {
				$data['int_marks'] = $int_marks[$key];
			}
    ...
    public function student_final_year_exam_detail(){
       ...
       $this->db->where_in('od.exam_year',array('June 2025','July 2025'));
        ...
        $this->db->where(array('cm.last_class'=>'L','od.university_mode'=>$data['mode'], 'od.marks_pattern'=>$data['pattern']));//'od.exam_status'=>'R',
	...
	public function student_final_year_exam_detail_pvt(){
        ...
        $data['students'] = $students;
        $title = ($data['mode'] == 'REG')?'Regular':'Private';
        $this->load->view('header',array('title'=> $title.' Student AISHE Data '.$data['exam_session'].' Marks Pattern'));
		$this->load->view('admin/student_final_year_exam_detail', $data);
		$this->load->view('footer');
    }
