...
// ));
	...
	{	
		...
		$query = $this->db->query("SELECT p.* FROM `paper_master` as p join class_master as c on c.id=p.class_id WHERE  type='Theory' and cbcs_paper=cbcs and class_id in (193,197,199,201,203,205,302,207,209,211,213,221,227,275,279,223,225,460,476) and ( class_name in ('I SEM','III SEM') or c.id in (155,182)) order by p.course_group_id,class_id,cbcs_paper,paper_pattern,paper_no asc");
