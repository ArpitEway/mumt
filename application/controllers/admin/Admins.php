...
public function update_exam_datewise_permission(){
	...
	public function generate_tr($mode="",$exam_pattern="M",$course_group_id="",$class_id="",$startlimit=0,$pagenumber=0){
		...
		//$this->db->where_in('roll_number',array(210710017,210710091,210710096,210710144,210710198,210710232,210710237,210710239,210710241,210710263,210710269,210710295,210710297,210710410,210710427,210710460,210710496,210710501,210710533,210710572,210710581,210710609,210710658,210710691,210710766,210710771,210710814,210710818,210710840,210710887,210710895,210710965,210711020,210711040,210711062,210711065,210711084,210711095,210711101,210711203,210711235,210711257,210711287,210711300,210711350,210711380,210711392,210711422,210711552,210711581,210711649,210711776,210711822,210711834,210711851,210711863,210711879,210711910,210712018,210712028,210712061,210712062,210712071,210712130,210712137,210712164,210712195,210712198,210712245,210712285,210712357,210712379,210712393,210712429,210712459,210712460,210712488,210712491,210712511,210712545,210712548,210712549,210712561,210712562,210712587));
	...
	{	
		...
		$query = $this->db->query("SELECT p.* FROM `paper_master` as p join class_master as c on c.id=p.class_id WHERE cbcs_paper=cbcs and type='theory'  order by p.course_group_id,class_id,cbcs_paper,paper_no asc");
...
public function forward_complaint(){
	...
	public function backlog_student_result_permission($mode="",$course_id="",$class_id=""){
		...
		$data['not_permited_students']= $this->Common_model->getRecordByWhere('backlog_student',array("course_group_id"=>$course_id ,'class_id' => $class_id , 'result_show'=>'N' , 'exam_form'=>'Y' ,'mode'=>$mode,'exam_year'=>'June 2023'));

		$data['permited_students']= $this->Common_model->getRecordByWhere('backlog_student',array("course_group_id"=>$course_id ,'class_id' => $class_id , 'result_show'=>'Y' , 'exam_form'=>'Y' ,'mode'=>$mode,'exam_year'=>'June 2023'));
		$data['name_csrf'] = $this->security->get_csrf_token_name();
		$data['hash_csrf'] = $this->security->get_csrf_hash();
		$data['mode']=$mode;
		$this->load->view('header',array('title' => ''));
	...
	public function backlog_student_notification_list($mode = "",$course_id="",$class_id=""){
		...
		$this->db->order_by('roll_no','ASC');
		$data['mode']= $mode;
		$data['students']= $this->Common_model->getRecordByWhere('backlog_student',array("course_group_id"=>$course_id ,'class_id' => $class_id,'exam_form'=>'Y' ,'roll_no!='=>'0', 'mode'=>$mode,'exam_year'=>"June 2023"));//'result_show'=>'Y'
		$data['title'] = "Notification ".$this->Common_model->getCourseNameByCourseId($course_id).' '.$this->Common_model->getClassNameByClassId($class_id);
	...
	public function backlog_student_marksheet($mode="",$course_id="",$class_id="",$startlimit=0)
	{
		...
		if($class->last_class == 'L'){
			...
			$this->db->join('student','student.student_id = backlog_student.student_id');
			$this->db->where(array("backlog_student.course_group_id"=>$course_id ,'backlog_student.class_id' => $class_id,'backlog_student.exam_form'=>'Y','backlog_student.roll_no!='=>'0','student.course_complete'=>'Y','backlog_student.mode'=>$mode,'backlog_student.result_show'=>'Y','backlog_student.exam_year'=>'June 2023'));
			...
			$this->db->from('backlog_student');
			$this->db->join('student','student.student_id = backlog_student.student_id');
			$this->db->where(array("backlog_student.course_group_id"=>$course_id ,'backlog_student.class_id' => $class_id,'backlog_student.exam_form'=>'Y','backlog_student.roll_no!='=>'0','backlog_student.mode'=>$mode,'backlog_student.result_show'=>'Y' ,'backlog_student.exam_year'=>'June 2023'));
