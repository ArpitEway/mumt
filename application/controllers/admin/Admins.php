...
public function getStudentData()
{
	...
	public function exam_form_status(){
		...
		$this->load->view('header',array('title' => 'Exam Form Status(Dec 2024)'));
		$data = array(
			'name_csrf' => $this->security->get_csrf_token_name(),
			'hash_csrf' => $this->security->get_csrf_hash(),
		);
...
public function update_exam_datewise_permission(){
	...
	public function backlog_generate_tr($mode="",$exam_pattern="M",$course_group_id="",$class_id="",$startlimit=0,$pagenumber=0){
		...
		$where =array("bs.course_group_id"=>$course_group_id ,'bs.class_id' => $class_id ,'bs.exam_form'=>'Y', 'bs.roll_no!='=>'0' ,'bs.mode'=> $mode,'bs.exam_year'=>'Dec 2024', 's.exam_pattern'=>$pattern);
    ...
    public function backlog_generate_tr_bed($mode="",$exam_pattern="M",$course_group_id="",$class_id=""){
        ...
        $where =array("bs.course_group_id"=>$course_group_id ,'bs.class_id' => $class_id ,'bs.exam_form'=>'Y', 'bs.roll_no!='=>'0' ,'bs.mode'=> $mode,'bs.exam_year'=>'Dec 2024','exam_pattern'=>$pattern);
		//,'student_id'=>702823
		$this->db->order_by('bs.center_id','ASC');
		$this->db->order_by('bs.roll_no','ASC');
	...
	public function tr_class_list(){
        ...
        $where = "id in (select distinct(course_group_id) from student where new_exam_form = 'Y' and class_id in (193,217,197,231,203,237,211,275,277,279,281,221,247,223,249,225,251,263,155,182,205,239,213,302,304,245,199,233,201,235,241,227,253,154,181,215,209,243) )";
	...
	public function backlog_tr_class_list(){
		$where = "id in (select distinct(course_group_id) from backlog_student where exam_form = 'Y' and exam_year='Dec 2024'  )";
	...
	public function internal_marks_list(){
		...
		$this->db->join('new_exam_form', 'student.student_id = new_exam_form.student_id and student.class_id=new_exam_form.class_id');
		...
		$this->db->where_in('new_exam_form.int_marks',array('ABS','N'));
		 $this->db->where_in('student.class_id',array(193,217,197,231,203,237,211,275,277,279,281,221,247,223,249,225,251,263,245,199,233,201,235,241,227,253,154,181,155,182,205,239,213,302,304));
		 //193,217,197,231,203,237,211,275,277,279,281,221,247,223,249,225,251,263,
    ...
    public function practical_marks_list(){
		...
		$this->db->join('class_master', 'student.class_id = class_master.id');
		$this->db->group_by('new_exam_form.student_id');
		$where = array('paper_type!='=>'theory','new_exam_form'=>'Y','p_marks_sub'=>'N');
		$this->db->Where($where);
	...
	public function student_notification_list_bed($mode="",$exam_pattern="M",$course_id="",$class_id="",$department=""){
		...
		$data['mode'] = $mode;
        // $class_cbcs = array(193,194,197,198,201,202,203,204,205,206,211,212,213,214,221,222,223,224,225,226,227,228,275,276,279,280);
        $class_cbcs = array(193,194,197,198,201,202,203,204,205,206,211,212,213,214,221,222,223,224,225,226,227,228,275,276,279,280,217,231,235,237,239,245,215,247,249,251,253,277,281,209,302,303,304,305);
...
//cbcs_paper=cbcs and 
   ...
   public function exam_center_login()
	...
	public function exam_center_billing_report(){
		...
		{
		$data = array();
		$data['title'] = "Exam Center Billing June 2024";
		$csrf = array(
			'name_csrf' => $this->security->get_csrf_token_name(),
			'hash_csrf' => $this->security->get_csrf_hash()
		);
...
public function forward_complaint(){
	...
	public function remove_backlog_student_result_permission(){
		...
		if($_POST['not_permitted']){
			$student_ids = (implode(',',$_POST['not_permitted']));
			$data = array('result_show' => 'Y');
			$where = 'student_id in ('.$student_ids.') and exam_year = "June 2024"';
			$update =$this->Common_model->updateRecordByConditions('backlog_student',$where,$data);
		}else{
			$student_ids = (implode(',',$_POST['permitted']));
			$data = array('result_show' => 'N');
	...
	public function backlog_student_marksheet($mode="",$course_id="",$class_id="",$startlimit=0)
	{
		...
		if($class->last_class == 'L'){
			...
			$this->db->join('student','student.student_id = backlog_student.student_id');
			$this->db->where(array("backlog_student.course_group_id"=>$course_id ,'backlog_student.class_id' => $class_id,'backlog_student.exam_form'=>'Y','backlog_student.roll_no!='=>'0','student.course_complete'=>'Y','backlog_student.mode'=>$mode,'backlog_student.result_show'=>'Y','backlog_student.exam_year'=>'June 2024','student.exam_pattern'=>'MARKS'));
			...
			$this->db->from('backlog_student');
			$this->db->join('student','student.student_id = backlog_student.student_id');
			$this->db->where(array("backlog_student.course_group_id"=>$course_id ,'backlog_student.class_id' => $class_id,'backlog_student.exam_form'=>'Y','backlog_student.roll_no!='=>'0','backlog_student.mode'=>$mode,'backlog_student.result_show'=>'Y' ,'backlog_student.exam_year'=>'June 2024','student.exam_pattern'=>'MARKS'));
	...
	public function backlog_student_previous_fail($mode = "",$course_id="",$class_id=""){
		...
		$this->db->select('bs.*, st.course_name, st.exam_pattern');
		$this->db->from('backlog_student as bs');
		$this->db->join('student as st', 'st.student_id = bs.student_id');
		$this->db->where(array('st.exam_pattern'=>'GRADE',"bs.course_group_id"=>$course_id ,'bs.class_id' => $class_id,'bs.exam_form'=>'Y' ,'bs.roll_no!='=>'0', 'bs.mode'=>$mode,'bs.result_show'=>'Y','bs.exam_year'=>'June 2024'));
		$data['students']=  $this->db->get()->result();
