...
class Admins extends CI_Controller {
	...
	public function get_papers_by_class_course()
	{
		...
		if ($this->input->method() == "post") 
		{
			...
			$this->db->order_by("course_group_id,class_id,cbcs_paper,paper_pattern,paper_no","asc");
...
public function getStudentData()
{
	...
	public function exam_form_status(){
		...
		$this->load->view('header',array('title' => 'Exam Form Status(June 2026)'));
		$data = array(
			'name_csrf' => $this->security->get_csrf_token_name(),
			'hash_csrf' => $this->security->get_csrf_hash(),
		);
	...
	public function class_wise_exam_from_status(){
		...
		$this->load->view('header',array('title' => 'Class Wise Exam Form Status(June 2026)'));
		$data = array(
			'name_csrf' => $this->security->get_csrf_token_name(),
			'hash_csrf' => $this->security->get_csrf_hash(),
		);
	...
	public function class_wise_old_exam_from_status(){
		...
		$this->load->view('header',array('title' => 'Class Wise Exam Form Status(Dec 2025)'));
		$data = array(
			'name_csrf' => $this->security->get_csrf_token_name(),
			'hash_csrf' => $this->security->get_csrf_hash(),
		);
	...
	public function class_wise_old_backlog_exam_form_status(){
		...
		$this->load->view('header',array('title' => 'Class Wise Backlog Exam Form Status(Dec 2025)'));
		$data = array(
			'name_csrf' => $this->security->get_csrf_token_name(),
			'hash_csrf' => $this->security->get_csrf_hash(),
		);
		$where = array('exam_form !=' =>'D' , 'exam_year'=>'Dec 2025');
	...
	public function class_wise_backlog_exam_from_status(){
		...
		$this->load->view('header',array('title' => 'Class Wise Backlog Exam Form Status(June 2026)'));
		$data = array(
			'name_csrf' => $this->security->get_csrf_token_name(),
			'hash_csrf' => $this->security->get_csrf_hash(),
		);
...
public function update_exam_datewise_permission(){
	...
	public function add_new_txn(){		
        ...
        if($Fess_head == "Backlog Exam Fees"){
            $this->db->order_by("id",'desc');
			$where = array('student_id'=>$student_id,'exam_year'=>'June 2026');
	...
	public function generate_tr($mode="",$exam_pattern="M",$course_group_id="",$class_id="",$startlimit=0,$pagenumber=0){
		...
		//, 'student_id'=>718196	, 'result_show'=>'N'
		...
		// $class_cbcs = array(193,194,197,198,201,202,203,204,205,206,211,212,213,214,221,222,223,224,225,226,227,228,275,276,279,280);
        $class_cbcs = array(193,194,197,198,201,202,203,204,205,206,211,212,213,214,221,222,223,224,225,226,227,228,275,276,279,280,217,231,235,237,239,245,215,247,249,251,253,277,281,209,302,303,304,305,278,282,250,252,216,232,236,238,240,246,248,254,218,305,210,243,267,244,268,269,312,313,207,476,332,432,502,506,510);
		$fourth_year = array(325,329,328);
		...
		else if( $class_id==256 || $class_id==257 || $class_id==316 ) {
...
// ));
        ...
        $class_cbcs = array(193,194,197,198,201,202,203,204,205,206,211,212,213,214,221,222,223,224,225,226,227,228,275,276,279,280,217,231,235,237,239,245,215,247,249,251,253,277,281,209,302,303,304,305,278,282,250,252,216,232,236,238,240,246,248,254,218,305,210,267);
	...
	public function tr_class_list(){
		...
		// $where = "id IN (SELECT DISTINCT course_group_id FROM student WHERE exam_form = 'Y' AND examcentercode IN ('MDE028','MDE172') AND exam_form = 'Y' AND old_class_id IN (103,106,109,112,121,127,130,133,136,148))";		

		$where = "id IN (SELECT DISTINCT course_group_id FROM student WHERE exam_form = 'Y' AND  old_class_id IN (267,269,255,259,257,316,432,502,506,510,215,217,229,231,233,235,237,239,245,304,277,281,247,249,251,253,197,201,203,211,213,275,279,221,223,225,302,476,205,207,209,154,181,263,261))";
		// 267,269,261,263,332,432,502,506,510,316

  		// 255,257,259,316,267,269	
		// 476,209,207,205,261,263,154,181
		
		// $where = "id in (select DISTINCT(course_group_id) from student where new_exam_form = 'Y' and class_id in (197,201,203,211,213,221,223,225,275,279,302,217,229,231,233,235,237,239,241,243,245,215,304,277,281,247,249,251,253))";and old_class_id in (154,155,181,182,193,217,195,197,231,199,233,201,235,203,237,205,239,241,209,243,211,245,213,215,302,304,275,277,279,281,221,247,223,249,225,251,227,253) 
        ...
        // 105,108,135
		// and class_id in (103,105,106,108,109,112,121,127,130,133,135,136,148,284,286,288,290,292,294,296,298,311)
	...
	public function student_result_permission($mode="",$course_id="",$class_id=""){
		...
		$data['not_permited_students']= $this->Common_model->getRecordByWhere('student',array("course_group_id"=>$course_id ,'old_class_id' => $class_id , 'result_show'=>'N' , 'exam_form'=>'Y' ,'university_mode'=>$mode ));
		// ,'examcentercode'=>'MDE165'
		$data['permited_students']= $this->Common_model->getRecordByWhere('student',array("course_group_id"=>$course_id ,'old_class_id' => $class_id , 'result_show'=>'Y' , 'exam_form'=>'Y' ,'university_mode'=>$mode));
		$data['name_csrf'] = $this->security->get_csrf_token_name();
		$data['hash_csrf'] = $this->security->get_csrf_hash();
		$data['mode']=$mode;
    ...
    public function backlog_student_marksheet_grade($mode="",$course_id="",$class_id="",$startlimit=0)
	{
		...
		if($class->last_class == 'L'){
            ...
            $this->db->from('backlog_student as bs');
            $this->db->join('student as st','st.student_id=bs.student_id');
            $this->db->where(array("bs.course_group_id"=>$course_id ,'bs.class_id' => $class_id,'bs.exam_form'=>'Y','bs.roll_no!='=>'0','mode'=>$mode,'bs.result_show'=>'Y','st.exam_pattern'=>"GRADE",'bs.exam_year'=>'Dec 2025' ));
            $data['students']=  $this->db->get()->result();
	...
	public function student_notification_list_bed($mode="",$exam_pattern="M",$course_id="",$class_id="",$department=""){
		...
		$data['mode'] = $mode;
        // $class_cbcs = array(193,194,197,198,201,202,203,204,205,206,211,212,213,214,221,222,223,224,225,226,227,228,275,276,279,280);
        $class_cbcs = array(193,194,197,198,201,202,203,204,205,206,211,212,213,214,221,222,223,224,225,226,227,228,275,276,279,280,217,231,235,237,239,245,215,247,249,251,253,277,281,209,302,303,304,305,267,268,312,313,269);
    ...
    public function backlog_student_notification_list_bed($mode="",$exam_pattern="M",$course_id="",$class_id=""){
        ...
        $this->db->from('backlog_student as bs');
        $this->db->join('student as s', 's.student_id = bs.student_id');
        $this->db->where(array("bs.course_group_id"=>$course_id ,'bs.class_id' => $class_id,'bs.exam_form'=>'Y' ,'bs.roll_no!='=>'0', 'bs.mode'=>$mode,'bs.exam_year'=>"Dec 2025", 's.exam_pattern'=>$pattern,'bs.result_show'=>'Y'));
        $data['students']= $this->db->get()->result();
		...
		$data['title'] = "Notification ".$this->Common_model->getCourseNameByCourseId($course_id).' '.$this->Common_model->getClassNameByClassId($class_id);
		$data['mode'] = $mode;
        $class_cbcs = array(193,194,197,198,201,202,203,204,205,206,211,212,213,214,221,222,223,224,225,226,227,228,275,276,279,280,217,231,235,237,239,245,215,247,249,251,253,277,281,209,302,303,304,305,267,268,282,312,313);
        if((in_array($class_id, $class_cbcs)) && $pattern=="GRADE"){
			$this->load->model('Gradesheet_backlog_tr_model_pg');
			$this->load->model('GradeSheet_old_model_pg');
	...
	{	
		...
		$query = $this->db->query("SELECT p.* FROM `paper_master` as p join class_master as c on c.id=p.class_id WHERE type='Theory' and cbcs_paper=cbcs and paper_pattern!='OLD' and class_name!='II SEM' and (`exam_form_permission`='Y' or `backlog_exam_form_permission`='Y')  order by p.course_group_id,class_id,cbcs_paper,paper_pattern,paper_no asc");
   ...
   public function exam_center_login()
	...
	public function select_paper($student_id){
		...
		if($student['session'] >='July 2024' && in_array($student['class_id'] , [267,268,269,273,223])){
	...
	public function class_wise_remaining_student_report(){
			...
			$set = array_column($classes,'id');
				...
				$this->db->order_by('student.course_group_id','student.class_id','student.university_mode','student.roll_number');
	...
	public function time_table(){
		...
		$this->db->where('paper_master.type','theory'); 
	   	$this->db->where_in('paper_master.class_id',[172,181,193,195,197,199,201,203,205,207,209,211,213,215,217,221,223,225,227,229,231,233,235,237,239,241,243,245,247,249,251,253,275,277,279,281,302,304,404,406,408,410,412,414,416,418,420,422,424,426,428,430,432,434,436,438,440,442,444,446,448,450,452,454,456,458,460,462,464,466,468,470,472,474,476,478,480,482,484,486,488,490,492,497,502,504,506,508,510,512,155,182,154,261,263,267,269,332,264,270,255,257,259,316,314,495]);
...
public function forward_complaint(){
	...
	public function backlog_student_result_permission($mode="",$course_id="",$class_id=""){
		...
		$data['not_permited_students']= $this->Common_model->getRecordByWhere('backlog_student',array("course_group_id"=>$course_id ,'class_id' => $class_id , 'result_show'=>'N' , 'exam_form'=>'Y' ,'mode'=>$mode,'exam_year'=>'Dec 2025'));

		$data['permited_students']= $this->Common_model->getRecordByWhere('backlog_student',array("course_group_id"=>$course_id ,'class_id' => $class_id , 'result_show'=>'Y' , 'exam_form'=>'Y' ,'mode'=>$mode,'exam_year'=>'Dec 2025'));
		$data['name_csrf'] = $this->security->get_csrf_token_name();
		$data['hash_csrf'] = $this->security->get_csrf_hash();
		$data['mode']=$mode;
		$this->load->view('header',array('title' => ''));
	...
	public function update_backlog_student_result_permission(){
		...
		if($_POST['not_permitted']){
			...
			$where = " student_id in (".$student_ids.") and exam_year = 'Dec 2025' and class_id=".$_POST['class_id']."";//provisional_remark in ('','N') &&
			$update =$this->Common_model->updateRecordByConditions('backlog_student',$where,$data);
		}else{
			$student_ids = (implode(',',$_POST['permitted']));
			$data = array('result_show' => 'N');
			$where ='student_id in ('.$student_ids.') and exam_year = "Dec 2025" and class_id='.$_POST["class_id"].'';
			$update = 	$this->Common_model->updateRecordByConditions('backlog_student',$where,$data);
		}  
		if($update){
	...
	public function remove_backlog_student_result_permission(){
		...
		if($_POST['not_permitted']){
			$student_ids = (implode(',',$_POST['not_permitted']));
			$data = array('result_show' => 'Y');
			$where = 'student_id in ('.$student_ids.') and exam_year = "Dec 2025"';
			$update =$this->Common_model->updateRecordByConditions('backlog_student',$where,$data);
		}else{
			$student_ids = (implode(',',$_POST['permitted']));
			$data = array('result_show' => 'N');
	...
	public function backlog_student_marksheet($mode="",$course_id="",$class_id="",$startlimit=0)
	{
		...
		if($class->last_class == 'L'){
			...
			$this->db->join('student','student.student_id = backlog_student.student_id');
			$this->db->where(array("backlog_student.course_group_id"=>$course_id ,'backlog_student.class_id' => $class_id,'backlog_student.exam_form'=>'Y','backlog_student.roll_no!='=>'0','student.course_complete'=>'Y','backlog_student.mode'=>$mode,'backlog_student.result_show'=>'Y','backlog_student.exam_year'=>'Dec 2025','student.exam_pattern'=>'MARKS'));
			...
			$this->db->from('backlog_student');
			$this->db->join('student','student.student_id = backlog_student.student_id');
			$this->db->where(array("backlog_student.course_group_id"=>$course_id ,'backlog_student.class_id' => $class_id,'backlog_student.exam_form'=>'Y','backlog_student.roll_no!='=>'0','backlog_student.mode'=>$mode,'backlog_student.result_show'=>'Y' ,'backlog_student.exam_year'=>'Dec 2025','student.exam_pattern'=>'MARKS'));
	...
	public function backlog_student_previous_fail($mode = "",$course_id="",$class_id=""){
		...
		$this->db->from('backlog_student as bs');
		$this->db->join('student as st', 'st.student_id = bs.student_id');
		$this->db->where(array('st.exam_pattern'=>'GRADE',"bs.course_group_id"=>$course_id ,'bs.class_id' => $class_id,'bs.exam_form'=>'Y' ,'bs.roll_no!='=>'0', 'bs.mode'=>$mode,'bs.result_show'=>'Y','bs.exam_year'=>'Dec 2025'));
		$data['students']=  $this->db->get()->result();
	...
	public function remove_backlog_student_result_permission_previous(){
		...
		if($_POST['not_permitted']){
			$student_ids = (implode(',',$_POST['not_permitted']));
			$data = array('result_show' => 'Y');
			$where = 'student_id in ('.$student_ids.') and exam_year = "Dec 2025"';//provisional_remark in ('','N') &&
			$update =$this->Common_model->updateRecordByConditions('backlog_student',$where,$data);
		}else{
			$student_ids = (implode(',',$_POST['permitted']));
			$data = array('result_show' => 'N');
