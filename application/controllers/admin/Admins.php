...
public function update_exam_datewise_permission(){
	...
	public function student_notification_list($mode = "",$exam_pattern="M",$course_id="",$class_id="",$startlimit=0,$pagenumber=0,$department=""){
        ...
        $class_cbcs = array(193,194,197,198,201,202,203,204,205,206,211,212,213,214,221,222,223,224,225,226,227,228,275,276,279,280,217,231,235,237,239,243,245,215,247,249,251,253,277,281,209,302,303,304,305,278,282,250,252,216,232,236,238,240,246,248,254,218,305,210);
	...
	//	$this->db->where_in('roll_number',array(210414167,210414296,210416271,210416325,210417759,210420469,210412133,210420009,210420534,210412125,210413275,210417990));
		...
		// $this->Common_model->last_query();
	 	// if($class->internal=="Y" && $mode!="PVT"){
			$class_cbcs = array(193,194,197,198,201,202,203,204,205,206,211,212,213,214,221,222,223,224,225,226,227,228,275,276,279,280,217,231,235,237,239,243,245,215,247,249,251,253,277,281,209,302,303,304,305,278,282,250,252,216,232,236,238,240,246,248,254,218,305,210,267);
	...
	{	
		...
		$query = $this->db->query("SELECT p.* FROM `paper_master` as p join class_master as c on c.id=p.class_id WHERE  type='Theory' and (( c.exam_form_permission='Y' or c.backlog_exam_form_permission='Y')) and c.class_name='I Year' order by p.course_group_id,class_id,cbcs_paper,paper_no asc");

		// and `exam_date`!='0000-00-00' and cbcs_paper=cbcs exam_date!='0000-00-00' and 
