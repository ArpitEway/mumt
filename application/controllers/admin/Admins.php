...
public function update_exam_datewise_permission(){
	...
	public function tr_class_list(){
		$where = "id in (select distinct(course_group_id) from student where new_exam_form = 'Y' and class_id in (155,182,296,288,292,262,268,270,300,294,286,298,256,258,274))";
		/******Final TR*******/
	...
	public function student_notification_list_bed($mode="",$course_id="",$class_id=""){
		...
		$this->db->order_by('roll_no','ASC');
		$data = array('course_group_id' => $course_id, 'class_id' => $class_id);
		$data['students']= $this->Common_model->getRecordByWhere('student',array("course_group_id"=>$course_id, 'class_id' => $class_id, 'new_exam_form'=>'Y','roll_no!='=>'0','university_mode'=>$mode,'roll_no'=>'126210023' ));
		$data['title'] = "Notification ".$this->Common_model->getCourseNameByCourseId($course_id).' '.$this->Common_model->getClassNameByClassId($class_id);
		$data['mode'] = $mode;
