...
public function update_exam_datewise_permission(){
	...
	public function tr_class_list(){
		...
		$where = "id in (select distinct(course_group_id) from student where exam_form = 'Y' and old_class_id in (267,269,263,261,255,257,172,259,193,195,229,199,233,205,239,302,207,241,209,243,211,245,213,215,223,225,227,253,299) )  ";
	...
	{	
		...
		$query = $this->db->query("SELECT p.* FROM `paper_master` as p join class_master as c on c.id=p.class_id WHERE cbcs_paper=cbcs and type='theory' and  type='Theory' and ( c.exam_form_permission='Y' or c.backlog_exam_form_permission)  order by p.course_group_id,class_id,cbcs_paper,paper_no asc");
