...
public function update_exam_datewise_permission(){
	...
	public function generate_tr($mode="",$exam_pattern="M",$course_group_id="",$class_id="",$startlimit=0,$pagenumber=0){
		...
		// $this->db->where_in('student_id',array(757903,756134,749879));
		// $data['students'] = $this->Common_model->getRecordByWhere('student_result_aug_22',$where);
		// $this->db->where_in('student_id',array(707815,726089,741118,749960,709939,726186,741334,750069,710095,726862,742728,750291,710139,727636,743721,750754,710671,732366,743985,751680,711264,732691,744379,751878,715689,733869,744706,753682,718623,734463,745183,754424,720719,734618,745542,754455,720792,735756,745758,756471,722414,735973,745797,756817,724175,737491,746082,756883,724433,738675,747754,758189,724771,740099,748325,725022,740144,748367,698052,725781,740754,749804));
...
// ));
	...
	public function tr_class_list(){
        ...
        $where = "id in (select distinct(course_group_id) from student where exam_form = 'Y' )";
        	...
        	//  and class_id in (168,107,110,104,119,125,128,131,134,101,164,173,177,171)
	...
	public function internal_marks_list(){
		...
		$this->db->where_in('new_exam_form.int_marks',array('ABS','N'));
		 $this->db->where_in('student.old_class_id',array(101,102,103,194,196,198,200,202,204,206,210,212,214,303,276,280,222,224,226,228,121,112,136,109,127,130,133,106,104,107,110,111,104,119,120,125,126,128,129,131,132,134,108,105));
    ...
    public function practical_marks_list(){
		...
		$this->db->where_in('student.old_class_id',[101,102,103,194,196,198,200,202,204,206,210,212,214,303,276,280,222,224,226,228,121,112,136,109,127,130,133,106,104,107,110,111,104,119,120,125,126,128,129,131,132,134,108,105]);
