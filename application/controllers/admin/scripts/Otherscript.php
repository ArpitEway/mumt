...
public function update_roll_no_old_data(){
	...
	public function dg_locker_data($startlimit=0){
		...
		$class_ids='101,104,107,110,116,119,125,128,131,134,102,105,108,111,117,120,126,129,132,135,103,106,109,112,118,121,127,130,133,136';
		 $sql="SELECT * FROM `old_exam_data` WHERE class_id in ($class_ids) and exam_year in ('".$exam_year."') and exam_result !='FAIL' and marks_pattern='GRADE' and  exam_status='R' and university_mode='PVT' order by course_name,class_id,roll_no";
		$rs = $this->db->query($sql)->result_array();
		$i=1;
