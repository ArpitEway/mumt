...
class Preexam extends CI_Controller {
	...
	public function new_exam_form_permission()
	{
		...
		$this->db->select('count(class_id) as num,course_name,class_name,class_id');
		$this->db->where('class_id in ('.$class_ids.') and new_exam_form="D" and new_admission_permission="N" and enrolled = "Y" and (( session="July 2023" and class_name = "I Year") || ( session="Jan 2024" and class_name = "I SEM")) ');
	...
	public function upload_group_exam_paper(){
		...
		$class_ids = $classes[0]['class_id'];
		//$class_ids =105; //105,216;
		$this->db->select('count(class_id) as num,course_name,class_name,class_id');
		$this->db->where('class_id in ('.$class_ids.') and temp_exam_form="N" and demo="N" and group_id!=""');
