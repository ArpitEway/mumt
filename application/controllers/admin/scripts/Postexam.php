...
class Postexam extends CI_Controller {
    ...
    function __construct(){
       ...
       public function upload_old_marks()
       {
            ...
            // 155,234,278,282,273,274,103,118,218,236,246,230,130,184,186,169,170,173,188,
             ...
             // 109,171,283,286,288,294,296
             // $this->db->where_in('class_id',array(168,107,110,104,119,125,128,131,134,101,164,173,177,171));
    ...
    public function upload_old_data_script($class_id="",$mode){
        ...
        $exam_year =  trim(preg_replace('/\s+/', ' ',str_replace('Examination', "", $date->exam_session)));
        $this->db->limit(500);
      ...
      //  $this->db->where('student_id',771494);
        ...
        foreach($students as $student)
        {
            ...
            $marksheetDate = ($student->university_mode == 'REG')?$date->result_date:$date->pvt_result_date;
             $marksheetDate=($marksheetDate == '')?"":DateTime::createFromFormat('d/m/Y', $marksheetDate)->format('Y-m-d');
           ...
           $aggregate_per = round(  ($tot_std_marks/$tot_marks) * 100,2);
            ...
            if($whCount!=0) {
                ...
                continue; //for witheld student result
            }
    ...
    public function promote_student_list(){
        ...
        $this->db->from('student_result_june_2025 as student');
      ...
      public function promote_student($class_id="" ,$course_group_id=""){
        ...
        $this->db->limit(1000);
          ...
          // $data['students']= $this->Common_model->getRecordByWhere('student',array('class_id' => $class_id, 'exam_form'=>'Y' , 'promote'=>'N' ,'course_complete'=>'N','university_mode'=>'REG' ));
      ...
      public function promote_student_submit(){
        foreach($_POST['student_id'] as $student_id){
           // $student= $this->Common_model->getRecordByWhere('student',array('student_id'=>$student_id));
            ...
            $update_result =$this->Common_model->updateRecordByConditions('student_result_june_2025',$where,$data_result);
        }
        if($update){
...
//For Backlog Fail student add in backlog exam in current exam Backlog to Backlog
    ...
    public function check_backlog_fail_student()
    {
      ...
      $this->db->where_in('id',array(155,182,299));
      // $this->db->where('backlog_exam_form_permission','Y');
      // $this->db->where_not_in('id',array(155,182));
      // $this->db->where_in('class_name',array('II SEM','IV SEM','VI SEM','VIII SEM'));
    ...
    public function backlog_marks_move_scripts($class_id='')
        {
           // $this->db->where('id>', '36941');
            ...
            $this->db->where_in('id',array(107114,107159,107223,107345,107348,107572,107603,107705,107715,107775,107931,107949,108173,108305,108400,108476,108527,108544,108577,108678,108685,108713,108721,108755,108814,108823,108828,108829,108830,131104,131226,131388,131461,131645,131652,131653,131851,197232,197233,197738,197746,197751,197774,197775,197784,197786,197787,197788,197789,197790,197860,197862,198984,198985,198986,198987,198988,198989,198990,198991,199131,199132,199133,199134,223380,223381,223382,223383,223384,239317,260703,260704,260705,260706,260707,260708,260709,260710,260711,260712,260714,260715,260716,260717,260718,260720,260721,260722,260723,260724,260725,260726,260727,260728,260729,260731,260732,260733,260734,260735,260736,260737,260738,260739,260740,260741,260742,260743,260744,260745,260747,260748,260749,260751,260752,260753,260754,260755,260757,260758,260759,260760,260761,260762,260763,260764,260765,260767,260768,260770,260771,260772,260773,260774,260775,260776,260777,260779,260780,260782,260784,261378,261379,261380,261381,261382,261383,261384,261385,261386,261387,261388,261389,261390,261474,261475,261476,261477,261478,261479,261480,263096));
            $studentall = $this->Common_model->getRecordByWhere("old_exam_data",array("class_id"=>$class_id,'exam_result'=>'Fail'));
            // ,'exam_status'=>'B'
            ...
            $old_result_datas = $this->Common_model->getRecordByWhere("old_result_data",$whereResult );
                $data = array(
                  ...
                  $duplicate =  $this->Common_model->getRecordByWhere('backlog_student',array('student_id'=>$students->student_id,'class_id'=>$students->class_id,'exam_year'=>'June 2026'));
                if( $duplicate !== Array ( )){
                    echo "Already Exist";
                  }else{
    ...
    public function set_demo($student_id,$class_id,$center_id)
    {
    ...
    public function backlog_marks_update_scripts($student_id,$class_id='')
    {
         ...
         // $this->db->where_in('exam_year',array('January 2025','February 2025'));
         $this->db->where_in('exam_year',array('June 2025','July 2025','August 2025'));
        $students = $this->Common_model->getRecordByWhere("old_exam_data",array("class_id"=>$class_id,"exam_status"=>'R','student_id'=>$student_id));
        //,'exam_year'=>'June 2024'
            ...
            if( $duplicate !== Array ( )){
                ...
                foreach($old_result_datas as $old_result_data)
                {
                    $examData = array(
                        ...
                        'paper_code' => $old_result_data->paper_code,
                        'paper_type' => $old_result_data->type,
                        'group_id' => $old_result_data->group_id,
                        'sub_group_id'=> $old_result_data->sub_group_id,
                        'paper_order' => $old_result_data->p_order,
                        'theory_marks' =>$old_result_data->theory_marks,
                        'int_marks' =>$old_result_data->int_marks,
                        'p_marks' => $old_result_data->p_marks,
     ...
     public function backlog_marks_add_scripts($student_id,$class_id,$exam_year,$id)
     {
       
        $exam_year = str_replace("%20"," ",$exam_year);
         $students = $this->Common_model->getRecordByWhere("old_exam_data",array("class_id"=>$class_id,'student_id'=>$student_id,'exam_year'=>$exam_year,'id'=>$id));
...
public function upload_old_grade_data_script_pg($class_id="",$mode){
  ...
  //  $this->db->limit(500);
    ...
    // $course_type = $this->Common_model->getRecordByWhere("course_group",array('id'=> $students[0]->course_group_id));
    // print_r($students);die;
    $this->load->model('Upload_old_data_pg');
    $x=1;
    foreach($students as $student)
    {
       ...
       echo $x.'<br>';
        // $this->upload_old_data->update_old_data($student->student_id,$student->course_group_id,$student->class_id,$student->university_mode);
        $this->Upload_old_data_pg->update_old_data($student);
     $x++;  
    }
...
public function upload_old_backlog_data_script($class_id="",$mode){
    ...
    public function upload_old_backlog_grade_data_script_pg($class_id,$mode){
        $classData = $this->Common_model->getRecordById('class_master','id',$class_id);
        $this->db->limit(500);
        ...
        $students = $this->db->get()->result();
        // print_r($students);die;
        $this->load->model('Upload_old_data_backlog_pg');
        $x=1;
        foreach($students as $student)
        {
           ...
           echo $x.'<br>';
            // $this->upload_old_data->update_old_data($student->student_id,$student->course_group_id,$student->class_id,$student->university_mode);
            $this->Upload_old_data_backlog_pg->update_old_data($student);
         $x++;  
        }
    }
