	...
	var _initWizard = function () {
		...
		_wizardObj.on('change', function (wizard) {
			...
			var student_id = $('#student_id').val();
				$.ajax({
			method: "POST",
			url: BASE_URL+"user/Users/getPapersByClassId",
			...
			Swal.fire({
				...
				if (result.value) {
					...
					$.ajax({
						method: "POST",
						async:false,
						url: BASE_URL+"user/saveFormdata/7",
	...
	function validation(step){
		...
		if(step==1){
			...
			if(!submit){
				...
				$.ajax({
					method: "POST",
					url: BASE_URL+"user/saveFormdata/1",
					data: "session="+session+'&course_group_id='+course_group_id+'&class_id='+class_id+'&eligibility='+eligibility+'&course_category='+course_category+'&student_id='+student_id,
					}).done(function( msg ) {
					return true;
				});
			}
			...
			if(!submit){
				...
				$.ajax({
					method: "POST",
					url: BASE_URL+"user/saveFormdata/2",
					data: formData,
					cache:false,
		            contentType: false,
		            processData: false,
			...
			*/
			...
			if(!submit){
				...
				$.ajax({
					method: "POST",
					url: BASE_URL+"user/saveFormdata/3",
			...
			$.ajax({
				method: "POST",
				url: BASE_URL+"user/saveFormdata/4",
				data: $('form[name="step_4"]').serialize()+'&student_id='+student_id,
				}).done(function( msg ) {
				return true;
			});
			}else if(step==5){
			...
			if(updatepaper=='Y'){
				...
				if(!groupVal){
					...
					$.ajax({
						method: "POST",
						url: BASE_URL+"user/saveFormdata/5",
			...
			if(submit){
				...
				$.ajax({
					method: "POST",
					url: BASE_URL+"user/saveFormdata/6",
					data: formData,
					cache:false,
					//async: false,
		            contentType: false,
