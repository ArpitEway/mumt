...
$("#course_group_id").on('change', function(){
	var course = $(this).val();
		$.ajax({
			method: "POST",
			url: BASE_URL+"admin/master/getClassAndEligibilityByCourse",
			data: {course : course},
			dataType: "json",
		})
		.done(function( msg ) {
...
$(".state").on('change', function(){
	...
	$.ajax({
		method: "POST",
		url: BASE_URL+"user/Users/getDistrictByState",
...
var KTWizard2 = function () {
			...
			Swal.fire({
				...
				var student_id = $('input[name="student_id"]').val();
					$.ajax({
						method: "POST",
						async:false,
						url: BASE_URL+"admin/updateStudentData/7",
	...
	function validation(step){
		...
		if(step==1){
			...
			if(course_category==''){
				$('input[name="course_category"]').next('div').text('Course Category is Required');
				submit = false
				}else{
				$('input[name="course_category"]').next('div').text('');
			}
			...
			var step1 =	$.ajax({
					method: "POST",
					url: BASE_URL+"admin/updateStudentData/1",
					data: "session="+session+'&course_group_id='+course_group_id+'&class_id='+class_id+'&eligibility='+eligibility+'&student_id='+student_id+'&course_category='+course_category,
					});
			...
			step1.done(function( data ) {
					...
					$.ajax({
						method: "POST",
						url: BASE_URL+"admin/updateStudentData/getPapersByClassId",
						data: {class_id : class_id, student_id:student_id},
					})
					.done(function( msg ) {
			            $('#PaperFormDiv').html(msg);
					});
					return true;
				});
			}
			...
			/* 
			if(blood_group==''){
				$('select[name="blood_group"]').next('div').text('Blood Group is Required');
				submit = false
				}else{
				$('select[name="blood_group"]').next('div').text('');
			}
			 if(adhar_no==''){
			...
			if(sm_id==''){
				$('input[name="sm_id"]').next('div').text('Samagra Id is Required');
				submit = false
				}else{
				$('input[name="sm_id"]').next('div').text('');
			} */
			if(!submit){
				return false;
				}else{
				...
				$.ajax({
					method: "POST",
					url: BASE_URL+"admin/updateStudentData/2",
					data: formData,
					cache:false,
		            contentType: false,
		            processData: false,
			...
			*/
			...
			if(!submit){
				...
				$.ajax({
					method: "POST",
					url: BASE_URL+"admin/updateStudentData/3",
					data: $('form[name="step_3"]').serialize(),
					}).done(function( msg ) {
					return true;
				});
			}
			...
			$.ajax({
				method: "POST",
				url: BASE_URL+"admin/updateStudentData/4",
				data: $('form[name="step_4"]').serialize(),
				}).done(function( msg ) {
				return true;
			});
			}else if(step==5){
			var updatepaper = $('#updatepaper').val();
			if(updatepaper=='Y'){
				var class_id = $('select[name="class_id"]').val();
				var student_id = $('input[name="student_id"]').val();
				var course_group_id = $('select[name="course_group_id"]').val();
				...
				for(var i=0; i<group_id.length;i++){
					if(group_id[i].checked == true){
						groupVal = true;    
					}
				}
				if(!groupVal){
					alert("Please Choose The Paper Group");
					return false;
					}else{
					$.ajax({
						method: "POST",
						url: BASE_URL+"admin/updateStudentData/5",
						data: $('form[name="step_5"]').serialize()+'&class_id='+class_id+'&course_group_id='+course_group_id+'&student_id='+student_id,
						}).done(function( msg ) {
						$('#PaperFormDiv').html(msg);
						return true;
					});
			...
			if(submit){
				...
				$.ajax({
					method: "POST",
					url: BASE_URL+"admin/updateStudentData/6",
					data: formData,
					cache:false,
					//async: false,
		            contentType: false,
...
$(".custom-file-input").on('change',function (){
		...
		if(data==''){
			...
			if(filesize>500){
				...
				$.ajax({
					url: BASE_URL+"admin/Document/uploadDoc",
					type: 'POST',
